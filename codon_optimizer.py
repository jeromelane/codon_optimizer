...
def program_setting():
    ...
    parser.add_argument('-reffreqtab', action='store', dest='reference_frequency_table',
                        default="",
                        help='Input path to the reference frequency table (default: Homo sapiens)')
    ...
    parser.add_argument('--version', action='version', version='codon optimizer 0.1')
    return parser
...
def codon_optimizer():
    ...
    if not os.path.exists(args.input):
        raise IOError("Please provide a fasta file containing sequences to analyze")

    if not args.nolog:
        logging.basicConfig(filename='codon_optimizer.log', level=logging.WARNING)
